...
+ (long long)dateToSeconds:(NSDate *)inputDate
{
    NSTimeInterval interval = [inputDate timeIntervalSince1970];
    long long seconds = interval * 1000;
    return seconds;
}

+ (NSString *)dateToCurrentDate:(long)strDate
{
    NSString *str = [[NSNumber numberWithLongLong:(long)strDate] stringValue];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyyMMdd"];
    NSDate *beginDate = [dateFormatter dateFromString:str];
    
//    NSTimeInterval timeInter = [[NSDate date] timeIntervalSinceDate:beginDate];
//    NSInteger days = (NSInteger)timeInter/(3600 * 24);
//    
//    
    NSCalendar *gregorian = [[NSCalendar alloc]initWithCalendarIdentifier:NSGregorianCalendar];
    NSUInteger unitFlags = NSMonthCalendarUnit | NSDayCalendarUnit;
    NSDateComponents *components = [gregorian components:unitFlags fromDate:beginDate toDate:[NSDate date] options:0];
    NSInteger totalMonth = [components month];
    NSInteger totalDay = [components day];
    
    NSInteger years = totalMonth / 12;
    NSInteger months = totalMonth % 12;
    NSInteger days = totalDay % 30;
    
    return [NSString stringWithFormat:@"宝宝%ld岁%ld月%ld天", (long)years, (long)months, (long)days];
}

+ (NSString *)dateToAge:(NSString *)dateAge
{
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyyMMdd"];
    NSDate *beginDate = [dateFormatter dateFromString:dateAge];
    ...
    NSInteger years = totalMonth / 12;
    NSInteger months = totalMonth % 12;
    NSInteger days = totalDay % 30 + 1;
    
    if (years == 0 && months == 0) {
        return [NSString stringWithFormat:@"%ld天", (long)days];
    }
    else if (years == 0 && months > 0) {
        return [NSString stringWithFormat:@"%ld月", (long)months];
    }
    else {
        return [NSString stringWithFormat:@"%ld岁%ld月", (long)years, (long)months];
    }
