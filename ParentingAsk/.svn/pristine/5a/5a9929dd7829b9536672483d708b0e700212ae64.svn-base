...
+ (id)objectFromDic:(NSDictionary *)dictionary className:(NSString *)name
{
    ...
    @try {
        ...
        for (int i = 0; i < count; i ++)
        {
            ...
            if ([dataType hasPrefix:@"c"]||[dataType hasPrefix:@"B"])
            {
                // BOOL
            ...
            for (int j = 0; j < count; j ++)
            {
                ...
                if (range.location == NSNotFound||range.length!=memberName.length-1) {
                    ...
                    NSString * oid = [NSString stringWithFormat:@"%@_id",classObject];
                    
                    id propertyValue = [dictionary objectForKey:propertyName];
                    
                    if([propertyName isEqualToString:oid])
                    {
                        propertyValue = [dictionary objectForKey:@"id"];
                    }
                    switch (rtype) {
...
+ (id)objectFromProperty:(NSArray *)propertyArray data:(NSArray *)dataArray className:(NSString *)name
{
    if (propertyArray     == nil || name == nil || name.length == 0||dataArray==nil) {
        return nil;
    }
        ...
        for (int i = 0; i < count; i ++)
        {
            ...
            for (int j = 0; j < count; j ++)
            {
                ...
                if (range.location == NSNotFound||range.length!=memberName.length-1) {
                    ...
                    NSInteger index = [propertyArray indexOfObject:propertyName];
                    
                    if ([oid isEqualToString:propertyName]) {
                        if (![propertyArray containsObject:@"id"]) {
                            continue;
                        }
                        index = [propertyArray indexOfObject:@"id"];
                        
                    }
                    else{
                        if (![propertyArray containsObject:propertyName]) {
                            continue;
                        }
                    }
                    
                    
                    id propertyValue = [dataArray objectAtIndex:index];
