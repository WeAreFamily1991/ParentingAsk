...
@interface AuditViewController ()<UINavigationControllerDelegate, UIImagePickerControllerDelegate, UIActionSheetDelegate, STPhotoKitDelegate>
{
...
-(void)httpRequest
{
    [HTTPRequest httpWithGet:@"/cus/detail" Param:@{} Success:^(id obj) {
        if (SUCCESS) {
            ...
            NSLog(@"得到obj =%@",obj);
            if ([[[obj valueForKey:@"data"] valueForKey:@"validateStatus"] isEqual:[NSNumber numberWithInt:3]])
            {
                ...
                MBHide  //[MBProgressHUD hideAllHUDsForView:[(UIViewController*)self view] animated:YES];
...
-(void)selectedRectangle5
{
    self.type =PhotoTypeRectangle5;
    [self editImageSelected];
}
...
-(void)uploadHeadImg2
{
    ...
    if (UIImagePNGRepresentation(self.upLoadIMG2) == nil) {
        ...
        imageData2 = UIImageJPEGRepresentation(self.upLoadIMG2, 1);
        
    } else {
        
        imageData2 = UIImagePNGRepresentation(self.upLoadIMG2);
    }
    NSLog(@"self.upLoadIMG1.image=%@",self.upLoadIMG2);
    if (imageData2) {
        NSLog(@"imageData=%@",imageData2);
        [HTTPRequest httpWithUploadPhoto:imageData2 Success:^(id obj) {
            if (!SUCCESS) {
                MBE(@"上传头像失败")
            }
            else if (SUCCESS)
...
-(void)uploadHeadImg3
{
    ...
    if (UIImagePNGRepresentation(self.upLoadIMG3) == nil) {
        ...
        imageData3 = UIImageJPEGRepresentation(self.upLoadIMG3, 1);
        
    } else {
        
        imageData3 = UIImagePNGRepresentation(self.upLoadIMG3);
    }
    NSLog(@"self.upLoadIMG1.image=%@",self.upLoadIMG3);
    if (imageData3) {
        NSLog(@"imageData=%@",imageData3);
        [HTTPRequest httpWithUploadPhoto:imageData3 Success:^(id obj) {
            if (!SUCCESS) {
                MBE(@"上传头像失败")
            }
            else if (SUCCESS)
...
-(void)uploadHeadImg4
{
    ...
    if (UIImagePNGRepresentation(self.upLoadIMG4) == nil) {
        ...
        imageData4 = UIImageJPEGRepresentation(self.upLoadIMG4, 1);
        
    } else {
        
        imageData4 = UIImagePNGRepresentation(self.upLoadIMG4);
    }
    NSLog(@"self.upLoadIMG1.image=%@",self.upLoadIMG4);
    if (imageData4) {
        NSLog(@"imageData=%@",imageData4);
        [HTTPRequest httpWithUploadPhoto:imageData4 Success:^(id obj) {
            if (!SUCCESS) {
                MBE(@"上传头像失败")
            }
            else if (SUCCESS)
...
-(void)uploadHeadImg5
{
    ...
    if (UIImagePNGRepresentation(self.upLoadIMG5) == nil) {
        ...
        imageData5 = UIImageJPEGRepresentation(self.upLoadIMG5, 1);
        
    } else {
        
        imageData5 = UIImagePNGRepresentation(self.upLoadIMG5);
    }
    NSLog(@"self.upLoadIMG1.image=%@",self.upLoadIMG5);
    if (imageData5) {
        NSLog(@"imageData=%@",imageData5);
        [HTTPRequest httpWithUploadPhoto:imageData5 Success:^(id obj) {
            if (!SUCCESS) {
                MBE(@"上传头像失败")
            }
            else if (SUCCESS)
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info
{
    [picker dismissViewControllerAnimated:YES completion:^{
        ...
        switch (self.type) {
            case PhotoTypeRectangle1:
                [photoVC setSizeClip:CGSizeMake(self.headIMG.width, self.headIMG.height)];
                break;
            case PhotoTypeRectangle2:
                [photoVC setSizeClip:CGSizeMake(self.identifyIMG.width, self.identifyIMG.height)];
                break;
            case PhotoTypeRectangle3:
...
-(void)GreatView
{
    ...
    UILabel *resonRigLab = [[UILabel alloc]initWithFrame:CGRectMake(ScreenWidth/4, ScreenHeight/15 +64, ScreenWidth/2, ScreenHeight/11)];
    resonRigLab.text =@"尚未提交审核";
    ...
    self.identifyTF.textAlignment = NSTextAlignmentLeft;
    ...
    self.LOCATF.textAlignment = NSTextAlignmentLeft;
...
-(void)saveBtnClick
{
    //@"/cus/validate" Param:@{@"card_name": self.nameTF.text, @"card_num": self.identifyTF.text, @"cer_no": self.LOCATF.text,@"headurl": imgName1,@"cardurl": imgName2,@"cerurl_front": imgName3,@"cerurl_back": imgName4,@"othurl": imgName5
...
-(void)authBtnClick
{
    ...
    //    }
    //    NSLog(@"imgCode=%@,imgCode2=%@,imgCode3 =%@",imgCode,imgCode2,imgCode3);
...
-(void)titBtnClick
{
    statusVC *statVC = [[statusVC alloc]init];
