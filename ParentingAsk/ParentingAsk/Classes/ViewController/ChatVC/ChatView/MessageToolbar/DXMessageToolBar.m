...
@interface DXMessageToolBar()<UITextViewDelegate, DXFaceDelegate, UITableViewDataSource, UITableViewDelegate>
{
    CGFloat _previousTextViewContentHeight;//上一次inputTextView的contentSize.height
    NSIndexPath *_indexMoreSelect;
}
...
//    _inputTextView.placeHolder = NSLocalizedString(@"message.toolBar.inputPlaceHolder", @"input a new message");
    [_inputTextView setPlaceHolder:@"育儿问一问"];
...
//        self.moreView = [[UITableView alloc] initWithFrame:CGRectMake(0, (kVerticalPadding * 2 + kInputTextViewMinHeight), self.frame.size.width, 80)];
        [(DXChatBarMoreView*)self.moreView setDelegate:self];
//        [self.moreView setDataSource:self];
    }
...
//    if ([_indexMoreSelect isEqual:indexPath]) {
...
//{
//    _indexMoreSelect = [NSIndexPath indexPathForRow:indexPath.row inSection:indexPath.section];
