...
@implementation UIViewController (VCPopUp)
...
//        viewControllerToPresent.view.layer.shadowColor = [UIColor blackColor].CGColor;
        viewControllerToPresent.view.layer.shadowRadius = 1.0f;
...
//            CGRect initialFrame = CGRectMake(finalFrame.origin.x, [UIScreen mainScreen].bounds.size.height + viewControllerToPresent.view.frame.size.height/2, finalFrame.size.width, finalFrame.size.height);
            CGRect initialFrame = CGRectMake(finalFrame.origin.x, -viewControllerToPresent.view.frame.size.height/2, finalFrame.size.width, finalFrame.size.height);
            viewControllerToPresent.view.frame = initialFrame;
            
            [self.view addSubview:viewControllerToPresent.view];
            UITapGestureRecognizer  *tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(blurviewTapClick:)];
...
-(void)blurviewTapClick:(UITapGestureRecognizer *)sender
{
    [self blurDismiss];
}
-(void)blurDismiss
...
//            self.popupViewController.view.frame = CGRectMake(initialFrame.origin.x, [UIScreen mainScreen].bounds.size.height + initialFrame.size.height/2, initialFrame.size.width, initialFrame.size.height);
            self.popupViewController.view.frame = CGRectMake(initialFrame.origin.x, -initialFrame.size.height / 2, initialFrame.size.width, initialFrame.size.height);
...
- (CGRect)getPopupFrameForViewController:(UIViewController *)viewController {
    ...
    if (UIInterfaceOrientationIsPortrait([UIApplication sharedApplication].statusBarOrientation)) {
        ...
        y = ([UIScreen mainScreen].bounds.size.height - frame.size.height + 44);
    } else {
        x = ([UIScreen mainScreen].bounds.size.height - frame.size.width)/2;
        y = ([UIScreen mainScreen].bounds.size.width - frame.size.height)/2;
    }
    return CGRectMake(x, y-64, frame.size.width, frame.size.height);
}

/**
 改变弹出控制器位置，将其添加至顶部
 **/
- (CGRect)getPopUpTopFrameForViewController:(UIViewController *)viewController
{
    CGRect frame = viewController.view.frame;
    if (frame.size.height == kScreenHeight-64-44) {
        return CGRectMake(0, 44, frame.size.width, frame.size.height);
    }
    else if (frame.size.height == kScreenHeight-64-49)
    {
...
- (void)presentPopupTopViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion
{
    if (self.popupViewController == nil) {
        ...
        self.popupViewController.view.autoresizingMask = UIViewAutoresizingNone;
        [self.popupViewController viewWillAppear:YES];
        CGRect finalFrame = [self getPopUpTopFrameForViewController:viewControllerToPresent];
        // parallax setup if iOS7+
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0) {
            UIInterpolatingMotionEffect *interpolationHorizontal = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.x" type:UIInterpolatingMotionEffectTypeTiltAlongHorizontalAxis];
        ...
        // setup
        if (flag) { // animate
            CGRect initialFrame = CGRectMake(0, 0, finalFrame.size.width, finalFrame.size.height);
            viewControllerToPresent.view.frame = initialFrame;
            [self.view addSubview:viewControllerToPresent.view];
            UITapGestureRecognizer  *tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(blurviewTapClick:)];
            [blurView addGestureRecognizer:tap];
            ...
            [UIView animateWithDuration:ANIMATION_TIME delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
                viewControllerToPresent.view.frame = finalFrame;
                blurView.alpha = self.useBlurForPopup ? 1.0f :  0.0f;
            } completion:^(BOOL finished) {
                [self.popupViewController viewDidAppear:YES];
                [completion invoke];
            }];
